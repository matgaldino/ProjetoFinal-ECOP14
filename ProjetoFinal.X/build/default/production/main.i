...
void desligado(void){
    ...
    lcdChar(((bcd2dec(dsReadData(5)& 0x3f)) % 10) + 48); lcdChar('/');
...
void atualizaLCD(void){
    ...
    lcdString("TEMP:");
    lcdPosition(1, 6);
    lcdChar((((250*pot)/1023)+50)/100 %10 + 48);
    lcdChar((((250*pot)/1023)+50)/10 %10 + 48);
...
void leTeclado(void){
    ...
    if(kpRead() != tecla){
        tecla = kpRead();
        if(((tecla) & (1<<(9)))){state = 1; lcdCommand(0x01);}
        if(state != 0){
            if(((tecla) & (1<<(5)))){modo = 5; min = 10; seg = 0; lcdCommand(0x01);}
            if(((tecla) & (1<<(6)))){modo = 6; min = 15; seg = 0; lcdCommand(0x01);}
            if(((tecla) & (1<<(8)))){modo = 8; lcdCommand(0x01);}
            if(((tecla) & (1<<(7)))){resist++; if(resist>2){resist = 0;}}

            if(start == 0){
                if(((tecla) & (1<<(0)))){min++; if(min>99){min=99;}}
                if(((tecla) & (1<<(2)))){min--; if(min<0){min=0;}}
                if(((tecla) & (1<<(3)))){seg++; if(seg>59){seg=0; min++; if(min>99){min=99;}}}
                if(((tecla) & (1<<(1)))){seg--; if(seg<0){seg=59; min--; if(min<0){min=0;}}}
            }
            if(((tecla) & (1<<(4)))){start = !start; fim=0;}
            if(modo!=0){
                atualizaLCD();
            }
        }
